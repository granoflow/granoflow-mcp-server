...
| Analysis close (UI tasks)              | Analysis deliverables                 | `task-work-document-workflow` § Analysis Deliverables (+ `prototype-doc-coverage` when UI)                            | `analysis_deliverables_incomplete`, `ui_prototype_required`, `milestone_task_plan_incomplete`                                                               |
| Multi-milestone Plan entry             | Pipeline order                        | `project-lifecycle-progress-board` Pipeline Order Gate (+ Project Work `pipeline_order`)                              | `pipeline_order_unresolved` when Plan starts while peers Analysis `not_started` and mode unset                                                              |
...
| UI product/technical Analysis closure  | Analysis finalization                 | `task-analysis-finalization` (+ logic, traceability, Grill, semantic, technical package linters)                      | `analysis_logic_draft_*`, `screen_content_contract_*`, `requirement_contract_*`, `contract_grill_*`, `contract_prototype_*`, `analysis_technical_package_*` |
...
| Plan Readiness / first edit / Delivery | Plan Design + structural + learning   | `plan-design-gate`, `software-structural-budget`, `implementation-learning-ledger`                                    | `plan_design_gate_*`, `structural_forecast_*`, `implementation_learning_*`, `acceptance_report_missing`                                                     |
| Project Definition Step 1 confirm      | Engineering pack                      | `engineering-acceptance-pack` (+ `markdown-html-acceptance-render`)                                                   | `engineering_acceptance_pack_*`, `init_ai_self_check_failed`, `directory_structure_unselected`, `visual_baseline_applicability_unresolved`                  |
...
| Milestone Plan done / implement        | Acceptance pack                       | `milestone-plan-acceptance-pack` (+ `markdown-html-acceptance-render`)                                                | `milestone_plan_acceptance_pack_*` (HTML missing is soft)                                                                                                   |
...
1. Project init (Engineering Acceptance Pack browse-confirm → App confirm;
   Spec/Shell user selection when UI) → 2. Milestones+tasks → 3. Analysis (**includes confirmed
   `ui_prototype` for UI-changing tasks**; emit remaining-deliverables list) → 4. Plan/Design Gate + acceptance pack → 5. Implement (Layer A + milestone
   Layer B IT; milestone delivery stops here—no E2E) → 6–7. **最终交付** per
   `granoflow_acceptance_delivery_skill` + `full-delivery-acceptance` (1 feature
   milestone → full-project E2E direct; ≥2 → unit + project IT + full-project
   E2E) → 8. Project complete. Plan Must not start ahead of that task's Analysis
   deliverables. Before first multi-milestone Plan entry, resolve Project Work
   `pipeline_order` (`breadth_first` vs `depth_first`) via the Pipeline Order
   Gate ask—do not assume depth-first.
