...
| Milestone acceptance (software)        | Milestone IT + matrix + static hygiene   | `milestone-integration-acceptance` + `static-quality-gate` (+ `lint_quality_gate_run.py`)                                                                                                                                                                           | `milestone_it_preflight_*` / `_coverage_*` / `_experience_*` / `_task_review_*`, `feature_completeness_*`, `functional_residual_forbidden`, `quality_gates_unconfigured`, `static_quality_gate_*`                                                                                  |
| Layer A / B / final (software)         | Static Quality Gate evidence             | `static-quality-gate` (+ `lint_quality_gate_run.py`)                                                                                                                                                                                                                | `quality_gates_unconfigured`, `static_quality_gate_skipped` / `_failed` / `_commands_mismatch` / `_scope_invalid` / `_unread`                                                                                                                                                      |
