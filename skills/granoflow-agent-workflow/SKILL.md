...
| Analysis close (UI tasks)              | Analysis deliverables                 | `task-work-document-workflow` § Analysis Deliverables (+ `prototype-serial-revision` + `prototype-doc-coverage` + `lint_prototype_link_ledger.py --require-complete` + `lint_prototype_revision_ledger.py` when UI) | `analysis_deliverables_incomplete`, `ui_prototype_required`, `prototype_link_*`, `prototype_revision_*`, `milestone_task_plan_incomplete`                                                                                                                  |
...
| UI prototype lock/rematch              | HTML/widget/doc coverage              | `prototype-doc-coverage` + `prototype-serial-revision`                                                                                                                                                              | `prototype_html_coverage_*`, `widget_reuse_required`, `prototype_plan_truth_*`, `prototype_revision_*`                                                                                                                                                     |
...
| Long/unattended implement/campaign     | Run continuity + Project E2E SoT      | `long-task-run-continuity` (**Unattended Entry Continuity Checklist**), `project-e2e-sot`, `project-e2e-sot-template` (+ `lint_project_e2e_sot.py --require-digest-match` on resume)                                | `long_task_continuity_*`, `long_run_plan_*`, `project_e2e_sot_*` (incl. stale digest match / missing gate lint in `next_step`), `host_wake_*` (Must probe; arm when available; `host_wake_unavailable_notice` + canonical resume; bare 「继续」 forbidden) |
...
1. Project init (Engineering Acceptance Pack browse-confirm → App confirm;
   ...
   **then** Implement (Layer A + Layer B) before the next milestone (Scheme 1).
   ...
   arms host wake bound to `temp/project-e2e-sot-v*.md` when the host exposes
