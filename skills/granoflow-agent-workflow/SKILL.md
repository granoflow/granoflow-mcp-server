...
| Analysis close (UI tasks)              | Analysis deliverables                 | `task-work-document-workflow` § Analysis Deliverables (+ `prototype-doc-coverage` + `lint_prototype_link_ledger.py --require-complete` when UI) | `analysis_deliverables_incomplete`, `ui_prototype_required`, `prototype_link_*`, `milestone_task_plan_incomplete`                                            |
...
| Milestone Plan done / implement        | Living acceptance pack                | `milestone-plan-acceptance-pack` (+ `markdown-html-acceptance-render` + `lint_milestone_plan_acceptance_pack.py --require-links`) | `milestone_plan_acceptance_pack_*`, `milestone_plan_test_lanes_incomplete`, `milestone_plan_prototype_alignment_failed`, `plan_acceptance_html_link_required` |
...
1. Project init (Engineering Acceptance Pack browse-confirm → App confirm;
   Spec/Shell user selection when UI) → 2. Milestones+tasks → 3. Analysis
   (**includes confirmed `ui_prototype` + link ledger for UI**; emit
   remaining-deliverables list; `gf析` may stop) → 4. Per-task Plan Design Gate
   + **living** milestone acceptance pack (HTML links, unit/IT/e2e Markdown
   lanes, prototype alignment). `gf规`/`run` **soft-merge** A→P without a
   courtesy “开始 Plan” pause → 5. Implement (Layer A + milestone Layer B IT;
   milestone delivery stops here—no E2E) → 6–7. **最终交付** per
   ...
   Gate ask—recommend depth_first for software UI long runs; never silently
   default.
