#!/usr/bin/env python3
"""Lint Plan Reality Boundary anti-drift: full index review + will_change refs."""
...
def _extract_index_fact_ids(snapshot: Any) -> set[str]:
    if not isinstance(snapshot, dict):
        return set()
    rows = snapshot.get("reality_boundary_index")
    ...
    *,
    ...
    if notice.get("none") is True:
        errors.append(
            _err(
                "card_change_plan_notice_missing",
                "planned card/RB changes forbid none:true",
            )
        )
    ...
    for index, item in enumerate(items):
        ...
        if kind == "reality_boundary" and not fact_id:
            errors.append(
                _err(
                    "card_change_plan_notice_missing",
                    f"{prefix}: reality_boundary items require fact_id",
                )
            )
...
def lint_plan_reality_boundary(
    data: Any,
    *,
    snapshot: Any | None = None,
    gate_required: bool = True,
) -> dict[str, Any]:
    """Validate Reality Boundary Plan fields on Task Work-shaped data."""
    errors: list[dict[str, str]] = []
    if not isinstance(data, dict):
        return {
            "ok": False,
            "code": "reality_boundary_lint_failed",
            "errors": [_err("reality_boundary_lint_failed", "task work must be an object")],
        }

    if not gate_required:
        return {"ok": True, "code": "ok", "errors": [], "gate": "not_applicable"}

    status_raw = data.get("reality_boundary_check_status")
    status = str(status_raw).strip() if _nonempty_str(status_raw) else ""
    review = data.get("reality_boundary_index_review")
    fact_ids = _as_str_list(data.get("reality_boundary_fact_ids"))
    will_change = _as_str_list(data.get("reality_boundary_will_change"))

    if status == "" or status == "missing":
        errors.append(
            _err(
                "reality_boundary_check_missing",
                "reality_boundary_check_status must be set "
    ...
    if status not in VALID_STATUS:
        errors.append(
            _err(
                "reality_boundary_check_missing",
                f"invalid reality_boundary_check_status: {status!r}",
            )
        )
    ...
    if status == "not_applicable":
        ...
        if not isinstance(review, list):
            errors.append(
                _err(
                    "reality_boundary_check_missing",
                    "reality_boundary_index_review must be a list when " "status is not_applicable",
                )
            )
        elif review:
            errors.append(
                _err(
                    "reality_boundary_check_missing",
                    "not_applicable requires reality_boundary_index_review: []",
                )
            )
        if not _nonempty_str(data.get("reality_boundary_not_applicable_reason")):
            errors.append(
                _err(
                    "reality_boundary_check_missing",
                    "not_applicable requires reality_boundary_not_applicable_reason",
                )
            )
    ...
    if not isinstance(review, list):
        errors.append(
            _err(
                "reality_boundary_check_missing",
                "reality_boundary_index_review must be a list",
            )
        )
        return {
            "ok": False,
    ...
    for index, row in enumerate(review):
        prefix = f"reality_boundary_index_review[{index}]"
        if not isinstance(row, dict):
            errors.append(_err("reality_boundary_check_missing", f"{prefix} must be an object"))
            continue
        ...
        if not fact_id or not FACT_ID_RE.match(fact_id):
            errors.append(
                _err(
                    "reality_boundary_check_missing",
                    f"{prefix}.fact_id must match RB-<slug>",
                )
            )
            continue
    ...
    if set(fact_ids) != set(derived_related):
        errors.append(
            _err(
                "reality_boundary_check_missing",
                "reality_boundary_fact_ids must equal related:true fact_ids "
    ...
    if set(will_change) != set(derived_will_change):
        errors.append(
            _err(
                "reality_boundary_check_missing",
                "reality_boundary_will_change must equal disposition=will_change "
    ...
    for fact_id in will_change:
        if fact_id not in fact_ids:
            errors.append(
                _err(
                    "reality_boundary_check_missing",
                    f"will_change fact_id not in reality_boundary_fact_ids: {fact_id}",
                )
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "task_work",
        type=Path,
        help="Task Work YAML/JSON with reality_boundary_* Plan fields",
    )
    parser.add_argument(
        "--snapshot",
        type=Path,
    ...
    try:
        ...
        result = lint_plan_reality_boundary(
        ...
        result = {
            "ok": False,
            "code": "reality_boundary_lint_failed",
            "errors": [_err("reality_boundary_lint_failed", str(error))],
        }
