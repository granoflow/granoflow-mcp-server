#!/usr/bin/env python3
"""Lint project E2E SoT: schema, stages, 3.1→3.2 pin, thin gates."""
...
def _load_frontmatter(path: Path) -> dict[str, Any]:
    ...
    if not text.startswith("---"):
        try:
            ...
            raise ValueError("SoT must start with YAML frontmatter or be JSON") from exc
        if not isinstance(data, dict):
            raise ValueError("JSON SoT root must be an object")
        return data
    end = text.find("\n---", 3)
    if end < 0:
...
def lint_project_e2e_sot(
    ...
    *,
    ...
    if data.get("doc_type") != DOC_TYPE:
        errors.append(_err("project_e2e_sot_lint_failed", f"doc_type must be {DOC_TYPE}"))
    schema = data.get("schema")
    if schema is not None and schema != SCHEMA:
        errors.append(
            _err(
                "project_e2e_sot_lint_failed",
    ...
    if not _nonempty_str(data.get("project_id")):
        errors.append(_err("project_e2e_sot_lint_failed", "project_id required"))
    if not _nonempty_str(data.get("updated_at")):
        errors.append(_err("project_e2e_sot_lint_failed", "updated_at required"))
    ...
    if not isinstance(stages, list):
        errors.append(_err("project_e2e_sot_lint_failed", "stages must be a list"))
        stages = []
    seen: set[str] = set()
    stage_by_id: dict[str, dict[str, Any]] = {}
    for row in stages:
        if not isinstance(row, dict):
            errors.append(_err("project_e2e_sot_lint_failed", "stage row must be object"))
            continue
        sid = row.get("id")
        if sid not in STAGE_IDS:
            ...
            continue
        if sid in seen:
            errors.append(_err("project_e2e_sot_lint_failed", f"duplicate stage id: {sid}"))
        seen.add(str(sid))
    ...
    for sid in STAGE_IDS:
        if sid not in seen:
            errors.append(_err("project_e2e_sot_lint_failed", f"missing stage id: {sid}"))

    work_items = data.get("work_items")
    if not isinstance(work_items, list):
        errors.append(_err("project_e2e_sot_lint_failed", "work_items must be a list"))
        work_items = []

    items_by_id: dict[str, dict[str, Any]] = {}
    for item in work_items:
        if not isinstance(item, dict):
            errors.append(_err("project_e2e_sot_lint_failed", "work_item must be object"))
            continue
        wid = item.get("id")
        if not _nonempty_str(wid):
            errors.append(_err("project_e2e_sot_lint_failed", "work_item.id required"))
            continue
        wid_s = str(wid)
        if wid_s in items_by_id:
            errors.append(_err("project_e2e_sot_lint_failed", f"duplicate work_item id: {wid_s}"))
        ...
        # Forbid per-task implement rows (milestone-level only: M1.implement OK)
        parts = wid_s.split(".")
    ...
    # 3.1 → 3.2 pin: any 3.1 done with sibling 3.2 not done ⇒ next must be that 3.2
    ...
    for wid, item in items_by_id.items():
        ...
        if not _nonempty_str(pair):
            # derive pair from id like M1.T1.3.1_analysis
            if ".3.1" in wid:
    ...
    if (
        ...
        errors.append(
            _err(
                "project_e2e_sot_next_step_unpinned",
    ...
    if not isinstance(itc, dict):
        errors.append(_err("project_e2e_sot_lint_failed", "integration_campaign object required"))
        itc = {}
    else:
    ...
    if not isinstance(e2e, dict):
        errors.append(_err("project_e2e_sot_lint_failed", "e2e_campaign object required"))
        e2e = {}
    else:
    ...
    # Optional coarse pointers; omit or [] is fine (backward compatible).
    ...
    if errors:
        ...
        return {"ok": False, "code": primary, "errors": errors}
    return {"ok": True, "code": "ok", "errors": []}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("path", type=Path, help="Path to project-e2e-sot markdown/JSON")
    parser.add_argument(
    ...
    try:
        data = _load_frontmatter(args.path)
    except (OSError, ValueError) as exc:
        result = {
            "ok": False,
            "code": "project_e2e_sot_lint_failed",
            "errors": [_err("project_e2e_sot_lint_failed", str(exc))],
        }
        print(json.dumps(result, ensure_ascii=False))
        return 1
    result = lint_project_e2e_sot(data, require_digest_match=args.require_digest_match)
    print(json.dumps(result, ensure_ascii=False))
