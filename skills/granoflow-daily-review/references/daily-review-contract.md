...
- Verify the running Granoflow Local HTTP API and inspect advertised daily
  review read/write capabilities before proposing a write. If unavailable, show
  a non-persistent draft and state the blocker.

## 1. Display

Use bounded evidence for the requested day: recorded tasks and timing, Task
Delivery, confirmed Task Reviews, project/milestone context, available focus or
session aggregates, existing daily-review values, and user statements supplied
in this conversation. Do not read hidden history or infer private events.
...
running App's supported Local HTTP API only, then read back the requested date.
Report each approved field as persisted, unavailable, conflict, or failed.
...
- write `taskReview` or alter task titles/status;
