...
Single owner for the **mutable, project-local orchestration SoT** stored at:

```text
temp/project-sot.yaml
```

It is the coarse main path from project init through delivery. Skill-internal
Grill, lint, and suite-merge details are black boxes—this file records gates,
`next_step`, and **evidence pointers only**.

Name note: this is **not** an E2E test artifact. Stage `e2e_campaign` is one
lifecycle gate among eight; campaign suites live under `temp/e2e-campaign/**`
as evidence.
...
`project_sot_unread`. Missing instance when required → `project_sot_missing`.
Stale digests / next step after cutoff → `project_sot_stale`.
...
Whole-file YAML (or JSON). No Markdown frontmatter for new writes.

Legacy `temp/project-e2e-sot-v*.md` is **read-only compatibility**. Lint emits
`project_sot_legacy_path`. Migrate to `temp/project-sot.yaml` before claiming
stage progress. Host wake binds `bound_run_plan_path` /
`bound_sot_path` to the active YAML.
...
Regen from App (when missing / wiped / refresh):
...
| Lost / stale                               | **Regen** from Granoflow App entities + digests; then lint                                            |

Do **not** create a full task-level SoT before `project_init` is done.

## App done gate (hard)

Acceptance closeout (artifact → confirm → checkbox) is owned by
`task-and-milestone-acceptance-layers`. On this SoT: do **not** mark a
task-linked `work_item` `done`, and do **not** advance `next_step` past that
task, until App readback for that task is `status=done`. Confirmed acceptance
and checkbox are the same wave—SoT must not run ahead of the App checkbox.

For milestone implement black boxes (`{M}.implement`): do **not** mark that
row `done` until Layer B is confirmed **and**
`lint_milestone_child_done.py --claim-passed` is green for that milestone.
...
4. Jumping `next_step` to another task while `T.3.2` is pending without
   `override` → `project_sot_next_step_unpinned`.
...
| Resume after cutoff / wake                            | `lint_project_sot.py --require-digest-match`                                                       |
...
`project_sot_next_step_missing_gate_lint` when a wake tick or stage claim
...
Campaign artifacts under `temp/e2e-campaign/**` and
`temp/integration-campaign/**` are **evidence only**. They must not pin
`next_step`. On interrupt, write campaign thin gates + `next_step` back to
`temp/project-sot.yaml` before stopping.

## Forbidden orchestration SoTs

Do **not** treat these as owners of `next_step`:

- `temp/unattended-run-ledger.yaml`
- `temp/residual-report.*`
- `temp/project-lifecycle-progress-board.md`
- `temp/**/campaign-state.json`
- coverage-matrix / closing-summary **bodies** (pointers only on SoT)
...
## Minimum document keys

See `project-sot-template.yaml`. Required keys:

- `doc_type: project_sot`
- `schema: granoflow_project_sot_v1` (when present)
...
- `next_step` with `work_item_id` when status is `active` and work remains
...
1. Run `regen_project_sot_from_app.py` (or equivalent) against App API.
2. Resolve project; read Project Work hash / milestones / tasks / pack statuses /
   campaign evidence pointers under `temp/`.
3. Rebuild stages + `work_items` from evidence (confirmed Analysis → 3.1 done,
   etc.).
4. Set `next_step` to the earliest legal unfinished item under Scheme 1 + pin
   rules.
5. Refresh `source_digests` and `updated_at`.
6. Lint before continuing.

Honest fields when evidence is missing (do not invent green):

- empty / `unknown` digests when App attachment absent
- stage `not_started` / thin gate `not_applicable` until proven
- `evidence_missing: true` notes on stages when `temp/` campaign dirs absent
...
| Regen CLI             | `regen-from-app.md`                                 |

## Admission Test

1. Was this reference loaded for a long / unattended project run?
2. Does `temp/project-sot.yaml` exist after `project_init`?
...
7. Is `next_step` absent from ledger / residual / board / campaign-state?
