...
const skill = readFileSync("skills/granoflow-milestone-coordination/SKILL.md", "utf8");
...
describe("milestone coordination contracts", () => {
  it("keeps coordination separate from portfolio and task authoring", () => {
    ...
    expect(skill).toMatch(/Do \*\*not\*\*[\s\S]*create the project milestone portfolio/i);
    expect(workflow).toContain("granoflow-task-authoring");
    expect(workflow).toMatch(/does not[\s\S]*batch-create milestones/i);
  });
  ...
  it("keeps milestone orchestration separate from child Task Work", () => {
    ...
    expect(skill).toContain("controller Task");
    expect(skill).toContain("Each child task owns its Task Work");
    expect(skill).toContain("Do not invent a complete Parallel Execution");
  ...
  it("phases Milestone Work fields and fills execute preflight after Analysis", () => {
    expect(template).toContain("Phased Required Fields");
    expect(template).toContain("charter_required");
    expect(template).toContain("decompose_required");
    expect(template).toContain("execute_preflight_required");
    expect(template).toContain("pending_task_analysis");
    expect(skill).toContain("execute_preflight_required");
    expect(workflow).toContain("Execute preflight after child Analysis");
  });
